...
public class TrapForest : MonoBehaviour
{
    ...
    public void TrapUpdate()
    {
        ...
        if (GameManager.Play.DS.timer <= 0)
        {
            ...
            {
                if (GameManager.Play.DC.lv != 1)
                {
                    LevelUP();
                }
                else
                {
                    GameManager.Play.DS.timer = float.MaxValue;
                    SetClear();
                }
            }
            else
    ...
    IEnumerator GoClear()
    {
        ...
        Camera.transform.position = Vector3.SmoothDamp(Camera.transform.position, new Vector3(-4, 0, 0), ref velocity, 1.0f);
        while (flag)
        {
            if (GameManager.Play.Player.transform.position.x < 30)
            {
