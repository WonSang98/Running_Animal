...
public class TrapForest : MonoBehaviour
{
    AudioClip clip;
    AudioClip clip2;
    AudioClip clip3;
    ...
    enum ID
    {
        ...
        Fly_Shot, // ¹Ð·Æ²ÛÀÇ Á¶¿ëÇÑ ÇÑ¹ß
        Monster,// Á¤·É?
        NoneJump_Stone,// °õÀâ´Â µ¹µ£
        NoneJump_Bee, // ¹úÀÌ ¹«¸®Áö¾îÀÖÀ½ (°øÁß) - Á¡ÇÁÇÏ¸é À¸¾Ó Áê±Ý
        Bridge_Stool, // ½£ Æ¯¼ö - ¹ßÆÇ
        Bridge_Trap // ½£ Æ¯¼ö - Àå¾Ö¹°
    }

    public GameObject[] traps; // ÇÔÁ¤ ¸®¼Ò½º ÀúÀå
    public GameObject warning_bird;
    ...
    public GameObject Parent; // ÇÔÁ¤À» ÀÌ ¿ÀºêÁ§Æ® ÇÏÀ§¿¡ »ý¼º½ÃÅ°±â À§ÇÔ.

    public int num_pattern = 255; // ÆÐÅÏÀÇ ¼ö.
    public void GetResource() // ÇÊ¿äÇÑ ¸®¼Ò½º °¡Á®¿À±â~
    {
    ...
    public IEnumerator TrapUpdate()
    {
        while (true)
        {
            if (GameManager.Play.DS.lvup == true) // ´ÙÀ½ ½ºÅ×ÀÌÁö·Î °¡´Â Á¶°ÇÀÌ ÃæÁ·µÇ¾úÀ»½Ã.
            {
            ...
            if (GameManager.Play.DS.timer <= 0)
            {
                ...
                Invoke("pattern" + GameManager.Play.DC.patternList[GameManager.Play.DC.patternCnt++], 0); // ÇÔÁ¤ È£Ãâ
                if (GameManager.Play.DC.patternCnt == (num_pattern + 1)) // ¸ðµç ÇÔÁ¤ÀÌ 1È¸¾¿ ´Ù ³ª¿ÔÀ» °æ¿ì.
                {
                    GameManager.Play.DC.patternCnt = 0;
    ...
    //trap_num ¼ø¼­ÀÇ ÇÔÁ¤À» pos¿¡ »ý¼ºÇÑ´Ù.
    ...
    {
        GameObject shot;
        GameObject warn = null; // ½Ç½Ã°£ ÁÂÇ¥ ¼öÁ¤
        float shot_y = 0;
        float time = 0;
    ...
    IEnumerator MakeSpecial()
    {
        ...
        for(int i=0; i<9; i++)
        {
            ...
            if (i == 0)
            {
                Brdige[i] = Instantiate(traps[(int)ID.Bridge_Stool]);
                Brdige[i].transform.position = new Vector3(start_x, bridge_y, 0);
                if (temp < 75)
                {
                    Trap[i] = Instantiate(traps[(int)ID.Bridge_Trap]);
    ...
    void pattern4() // ¹öµå½ºÆ®¶óÀÌÅ© »ý¼º
    {
        StartCoroutine(cotime(3, "MakeBird", 0.2f));
    }

    void pattern5() // ÃÑ ½î±â »ý¼º
    {
        StartCoroutine(cotime(3, "MakeShot", 0.2f));
    }

    void pattern6() // ¸ó½ºÅÍ »ý¼º
    {
    ...
    void LoadSound()
    {
        clip = Resources.Load<AudioClip>("Sound/Play/005_Play");
        clip2 = Resources.Load<AudioClip>("Sound/Play/009_Play");
        clip3 = Resources.Load<AudioClip>("Sound/Play/005_Play");
        clip3 = Resources.Load<AudioClip>("Sound/Play/008_Play");
    }
